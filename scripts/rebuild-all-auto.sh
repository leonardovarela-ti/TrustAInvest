...
echo -e "${YELLOW}TrustAInvest.com - Automated Complete System Rebuild Script${NC}"
echo -e "${YELLOW}=====================================================${NC}"
...
run_integration_tests() {
  # Check if we should run tests based on command line argument
  if [ "$1" == "--run-tests" ]; then
    ...
    echo -e "${YELLOW}Skipping integration tests. Use --run-tests flag to run them.${NC}"
  fi
}
...
main() {
  ...
  # Pass command line arguments to run_integration_tests
  run_integration_tests "$@"
}
