...
if [ -z "$1" ]; then
    ...
    echo "Usage: $0 <verification_request_id>"
    exit 1
fi
...
if docker-compose exec postgres psql -U trustainvest -d trustainvest -c "BEGIN; UPDATE kyc.verification_requests SET status = 'VERIFIED', updated_at = NOW() WHERE id = '$VERIFICATION_REQUEST_ID'; COMMIT;" | grep -q "UPDATE 1"; then
...
if docker-compose exec postgres psql -U trustainvest -d trustainvest -c "BEGIN; UPDATE users.users SET kyc_status = 'VERIFIED', updated_at = NOW() WHERE id = (SELECT user_id FROM kyc.verification_requests WHERE id = '$VERIFICATION_REQUEST_ID'); COMMIT;" | grep -q "UPDATE 1"; then
...
if docker-compose exec postgres psql -U trustainvest -d trustainvest -c "
